...
ist.issued_date,
-- rs.return_date,
CURRENT_DATE - ist.issued_date - 10167 as over_dues_days
...
WHERE (CURRENT_DATE - ist.issued_date - 10167)>30 
order by 1;
...
VALUES ('RS125', 'IS130', 'Good', CURRENT_DATE());
...
-- Step 2: Use custom delimiter for multiline procedure
DELIMITER $$
...
SELECT 
   ...
   COUNT(rs.issued_id) as number_of_book_return
FROM issued_status as ist
JOIN employees as e
 ON e.emp_id =ist.issued_emp_id
JOIN branch as b
...
BEGIN
    ...
    -- If book is available, proceed with issuing
    ...
    ELSE
        -- Simulate RAISE NOTICE
        SELECT CONCAT('Sorry, the book is currently unavailable. ISBN: ', p_issued_book_isbn) AS message;
    END IF;
END $$
